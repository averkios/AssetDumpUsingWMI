...
elseif ($NotFound)
{
    ...
    #Run until everything is found
    while ( ((Get-ChildItem $NotFoundPathRoot | Measure-Object ).Count) -ne 0)
    {
...
{
    Write-Host "To run this script for specific computers use parameter -Computers followed by the computer names (separated by commas)"
    Write-Host "To run this script for the not found computers use parameter -NotFound"
    Write-Host "WARNING : You will run this script for all the computers in the Domain.`nAre you sure ?"

    #Ask the user if he is sure. Until a valid answer is given.
    ...
    #Do apropriate actions depending on the answer
